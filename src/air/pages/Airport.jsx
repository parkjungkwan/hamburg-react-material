...
export const getAirports = data => {
    alert(`조회된 수: ${data.length}`)
    return ({type: "FETCH_AIRPORT", payload: data})}

export const airportReducer = (state = 0, action) =>{
    switch (action.type) {
...
export const Airport = () => {
    ...
    const selectAirport = payload => {
        setSelected(true)
        setResult(false)
        setAirport({name: payload.name, city: payload.city, country: payload.country})
    }
